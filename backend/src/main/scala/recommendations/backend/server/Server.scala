...
class Server extends ScalaVerticle {
  ...
  private def setupRoutes(router: Router): Unit = {
    ...
    router.get("/user/self/service_reviews")
      .handler(handleGetUserItemReviews(isRequestForServiceReviews = true)(_, dbManager))
    router.get("/user/self/place_reviews")
      .handler(handleGetUserItemReviews(isRequestForServiceReviews = false)(_, dbManager))
